...
export function isWav(data: Uint8Array | ArrayBuffer): boolean {
...
export function decodeWav(data: Uint8Array | ArrayBuffer): DecodedAudio {
  const view = toDataView(data);
  if (!isWav(data)) {
  ...
  if (
    !(
      (format === WAVE_FORMAT_PCM && [8, 16, 24, 32].includes(bitsPerSample)) ||
      (format === WAVE_FORMAT_IEEE_FLOAT && bitsPerSample === 32)
    )
  ) {
    throw new Error(
      `unsupported WAV format: format code ${format}, ` +
