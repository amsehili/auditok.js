 ...
 */
export function calculateEnergy(samples: ArrayLike<number>): number {
...
function resolveChannelIndex(index: number, channels: number): number {
  const selected = index < 0 ? index + channels : index;
  if (!Number.isInteger(index) || selected < 0 || selected >= channels) {
    throw new RangeError(
      `Selected channel must be >= -channels and < channels, given: ${index}`,
    );
  }
  return selected;
}
...
export function windowEnergy(
  channelData: readonly Float32Array[],
  useChannel: ChannelSelection = "any",
): number {
  ...
  if (useChannel === "mix") {
    return calculateEnergy(
      mixChannels(channelData, 0, channelData[0].length),
    );
  }
...
export function computeFrameEnergies(
  ...
  if (!Number.isInteger(frameSamples) || frameSamples <= 0) {
    throw new RangeError(
      `'frameSamples' must be a positive integer, given: ${frameSamples}`,
    );
  }
...
function histogram(
  ...
  min: number,
  max: number,
): { hist: Float64Array; edges: Float64Array } {
...
function estimateThresholdOtsu(energies: number[], bins = 128): number {
  let min = Infinity;
  let max = -Infinity;
  ...
  for (let i = 0; i < betweenVar.length; i++) {
    if (betweenVar[i] > best) best = betweenVar[i];
  }
  ...
  for (let i = 0; i < betweenVar.length; i++) {
    if (betweenVar[i] === best) candidates.push(i);
  }
...
function estimateThresholdPercentile(
  energies: number[],
  percentile: number,
  margin: number,
): number {
 ...
 */
export function estimateEnergyThreshold(
  frameEnergies: ArrayLike<number>,
  method: ThresholdMethod = "otsu",
  options: ThresholdMethodOptions = {},
): number {
  if (frameEnergies.length === 0) {
    throw new RangeError(
      "Cannot estimate an energy threshold from an empty energy array " +
  ...
  if (method === "percentile") {
    return estimateThresholdPercentile(
      energies,
      options.percentile ?? 10,
      options.margin ?? 6,
    );
  }
  throw new RangeError(
    `Unknown threshold estimation method '${method}', expected ` +
      `"otsu" or "percentile"`,
  );
}
